...
def generate_text_pplm(
    ...
    for i in trange(length, ascii=True):
        ...
        #print(len(pert_past))
        #print(len(pert_past[0]))
        #print(pert_past[0].shape)
...
def run_pplm_example(
    ...
    results = perplexity.compute(predictions=sampless, model_id='gpt2')
    
    print("Perturbed_layer_num=", perturb_layer)
    
    
    modelss = torch.load('model.pt')
    #print("sampless",sampless)
    #print(modelss)
    tokenizer = BertTokenizerFast.from_pretrained('bert-base-uncased')

    datas = tokenizer(sampless, return_tensors='pt', padding=True, truncation=True)
    #print(data)
    datas = { k: v.to(device) for k, v in datas.items() }
    with torch.no_grad():
        y_pred = modelss(datas)
    
    print("Sentiment label by bert=",y_pred.argmax(1))
    print("Sentiment_Accuracy = ", round(len(torch.where(y_pred.argmax(1) ==1)[0])/len(y_pred)*100,2))
    
    print("Perplexity=",round(results['mean_perplexity'] ,2))
    print("Dist_1= ",round(distinct_n_corpus_level(sampless, 1),2) )
    print("Dist_2= ",round(distinct_n_corpus_level(sampless, 2),2) )
    print("Dist_3= ",round(distinct_n_corpus_level(sampless, 3),2) )
    
    if perturb_layer == 100:
        ssa_x = [perturb_layer,
        unpert_gen_text,
        sampless,
        round(len(torch.where(y_pred.argmax(1) ==1)[0])/len(y_pred)*100,2),
        round(results['mean_perplexity'] ,2),
        round(distinct_n_corpus_level(sampless, 1),2),
        round(distinct_n_corpus_level(sampless, 2),2),
        round(distinct_n_corpus_level(sampless, 3),2)]
    ...
    with open(f'{perturb_layer}"{cond_text}"{stepsize}_devision_{num_samples}.pkl','wb') as f:
        pickle.dump(ssa_x,f)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--perturb_layer", type=int, default=100)
